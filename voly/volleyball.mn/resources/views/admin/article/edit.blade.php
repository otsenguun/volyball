...
<div id="content" class="app-content">
    <div class="d-flex align-items-center mb-3">
        <div>
            <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Мэдээ засварлах</li>
            </ul>
            <h1 class="page-header mb-0">{{ $article[0]->title }}</h1>
        </div>
        <div class="ml-auto">
            <a href="{{ url('/admin/articles') }}" class="btn btn-primary"><i class="fa fa-plus-circle fa-fw mr-1"></i>Буцах</a>
        </div>
    </div>
    <div class="container">
        <div class="row">
    ...
    <div class="card" style="padding: 25px;">
        <div class="container">
            <form action="{{ url('/admin/articles/edit/'.$article[0]->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-8">
                        <div class="form-group">
                            <label for="">Мэдээний гарчиг</label>
                            <input type="text" name="title" class="form-control" required="" value="@if(Session('backtitle')){{ Session('backtitle') }} @else {{ $article[0]->title }} @endif">
                        </div>
                        <div class="form-group">
                            <textarea name="article" class="summernote" id="contents" title="Contents" required="">
								@if(Session('backarticle')) {!! Session('backarticle') !!} @else {{ $article[0]->article }} @endif
							</textarea>
                        </div>
                    </div>
                    <div class="col-md-4">
                        ...
                        <div class="form-group">
                            <label for="">Мэдээний төрөл</label>
                            <select name="category" id="" class="form-control">
                                @foreach($categories as $category)
                                    <option value="{{ $category->category }}" @if(Session('backcategory')) @if(Session('backcategory') == $category->category) selected="" @endif @else @if($article[0]->category == $category->category) selected="" @endif @endif>{{ $category->category }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <a href="{{ url('/admin/articles/edit/'.$article[0]->id) }}" class="btn btn-warning">Reset хийх</a>
                        </div>
                    </div>
                </div>
            </form>
